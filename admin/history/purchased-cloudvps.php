		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<?php
					...
					// Tổng số bản ghi
					$total_records = $ketnoi->num_rows("SELECT * FROM `purchased_cloudvps`");
					
					// Tổng số trang
					$total_pages = ceil($total_records / $limit);
					
					// Lấy dữ liệu theo trang + sắp xếp
					$rows = $ketnoi->get_list("SELECT * FROM `purchased_cloudvps` $orderBy LIMIT $offset, $limit");
				...
				<div class="card">
					...
					<!-- ================= FILTER Purchased CloudVPS ================= -->
					<div class="card-body">
						<form id="filterPurchasedCloudVPSForm" class="row g-3 align-items-center flex-wrap justify-content-between">
							<!-- ID -->
					...
					<!-- ================= BẢNG QUẢN LÝ Purchased CloudVPS ================= -->
							...
							<table class="table table-striped mb-0">
								<thead class="table-light align-middle">
									<tr> 
										...
										<th><i class="fas fa-cube me-1 text-success"></i>Tên gói</th> 
										<th><i class="fas fa-sync-alt me-1 text-info"></i>Chu kì thanh toán</th> 
										<th><i class="fas fa-money-bill-wave me-1 text-success"></i>Tổng tiền</th>  
										<th><i class="fas fa-toggle-on me-1 text-primary"></i>Trạng thái</th> 
										<th><i class="fas fa-calendar-alt me-1 text-secondary"></i>Thời gian mua</th>
									</tr>
								</thead>
								<tbody id="purchasedCloudVPSTableBody">
									<?php if (!empty($rows)): ?>
									<?php foreach ($rows as $row): ?>
									<tr data-id="<?= $row['id'] ?>"
										data-username="<?= ($row['username']) ?>"
										data-id-produc="<?= ($row['id_produc']) ?>"
										data-id-vps="<?= ($row['id_vps']) ?>"
										data-billingcycle="<?= ($row['billingcycle']) ?>"
										data-money="<?= ($row['money']) ?>"
										data-status="<?= $row['status'] ?>"
										data-created-at="<?= $row['created_at'] ?>" >
										<?php 
											$id_product = (int)$row['id_produc'];  
										...
										<!-- id_produc -->
										<td class="fw-semibold">   
											<?= !empty($check_product['name']) ? ($check_product['name']) : 'N/A' ?> 
										</td>
										<!-- billingcycle -->
										<td class="fw-semibold"> 
											<?= !empty($row['billingcycle']) ? check_Month($row['billingcycle']) : 'N/A' ?> 
										</td>
										<!-- money -->
										<td class="fw-semibold"> 
											<?= !empty($row['money']) ? money($row['money']) : 'N/A' ?> đ 
										</td> 
										<!-- Trạng thái -->
										<td>
				...
				<script>
					...
					document.addEventListener("DOMContentLoaded", () => {
						...
						// 🔍 FILTER Purchased CloudVPS 
						// ==================================================  
						const tableBody = document.getElementById("purchasedCloudVPSTableBody");
						...
						if (tableBody) {
							...
							function applyFilter() {
								...
								const filtered = originalRows.filter(tr => {
									...
									// Lấy data từ attributes (ID, Username, Status)
									const data = {
									...
									// Lấy TÊN GÓI từ cột thứ 3 (cột Tên gói)
									const nameCell = tr.cells[2]; // Index 2 là cột "Tên gói"
									const productName = nameCell ? nameCell.textContent.trim().toLowerCase() : "";
									...
									// Name filter (tên gói) - tìm trong nội dung cột thứ 3
									if (values.name && !productName.includes(values.name)) return false;
							...
							if (filters.id) {
								filters.id.addEventListener('input', () => {
									clearTimeout(filters.id.timer);
									filters.id.timer = setTimeout(applyFilter, 300);
								});
							}
							
							if (filters.username) {
								filters.username.addEventListener('input', () => {
									clearTimeout(filters.username.timer);
									filters.username.timer = setTimeout(applyFilter, 300);
								});
							}
							
							if (filters.name) {
								filters.name.addEventListener('input', () => {
									clearTimeout(filters.name.timer);
									filters.name.timer = setTimeout(applyFilter, 300);
								});
							}
							
							if (filters.level) {
								filters.level.addEventListener('change', applyFilter);
							}
							...
							// Không cần gọi applyFilter() lần đầu nếu không muốn filter tự động
						}
