...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($action == "UPDATE_STATUS") {
		...
		$check = $ketnoi->get_row("SELECT * FROM `history_buy_hosting` WHERE `id` = '$id'");
		if (!$check) json_Msg("error", "Lịch sử này không tồn tại không tồn tại");

		$ketnoi->begin_transaction();
		$update = $ketnoi->update("history_buy_hosting",array("status" => $status), "`id` = '$id'");
